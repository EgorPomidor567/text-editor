...
def delete_text():
	if list_notes.selectedItems() and list_tags.selectedItems()[0].text():
		key = list_notes.selectedItems()[0].text()
		tags = list_tags.selectedItems()[0].text()
...
def search_by_tag():
	...
	if tag:
		matching_notes = [note[0] for note in notes if tag in notes[2]]
		list_notes.clear()
		list_notes.addItem(note[0])
	else:
		for note in notes:
